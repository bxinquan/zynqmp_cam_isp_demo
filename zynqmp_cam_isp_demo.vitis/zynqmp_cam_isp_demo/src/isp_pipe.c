...
static int v_frmbuf_rd_init(UINTPTR yptr, UINTPTR cptr)
{
	...
	Status = XVFrmbufRd_Initialize(&v_frmbuf_rd, XPAR_ISP_PIPE_VFRM_RD_ISP_DEVICE_ID);
...
static int v_frmbuf_wr_init(UINTPTR yptr, UINTPTR cptr)
{
	...
	Status = XVFrmbufWr_Initialize(&v_frmbuf_wr, XPAR_ISP_PIPE_VFRM_WR_ISP_DEVICE_ID);
	if(Status != XST_SUCCESS) {
