...
#define BUFFERSIZE			DISP_WIDTH * DISP_HEIGHT * 3		/* HTotal * VTotal * BPP */
#define LINESIZE			DISP_WIDTH * 3			/* HTotal * BPP */
#define STRIDE				ALIGN(DISP_WIDTH * 3, 256)			/* The stride value should
													be aligned to 256*/
...
static XAVBuf AVBuf;
//static XScuGic Intr;
static Run_Config RunCfg;

static u8 Frame[BUFFERSIZE] __attribute__ ((__aligned__(256)));
...
*
*****************************************************************************/
int DpdmaVideoExample(INTPTR buffer)
{
	...
	FrameBuffer.Address = (INTPTR)buffer;
...
*
*****************************************************************************/
static u8 *GraphicsOverlay(u8* Frame, Run_Config *RunCfgPtr)
{
	u64 Index;
	u32 *RGBA;
	RGBA = (u32 *) Frame;
