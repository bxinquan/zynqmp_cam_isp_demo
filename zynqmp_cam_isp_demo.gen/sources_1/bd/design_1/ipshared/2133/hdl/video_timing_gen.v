...
(
	...
	always @ (posedge vid_clk or negedge vid_rstn) begin
		if (!vid_rstn)
			pix_cnt <= 0;
		else if (pix_cnt == 16'd0 && line_cnt == 16'd0 && vid_frame_ready == 1'b0)
			pix_cnt <= pix_cnt;
	...
	always @(posedge vid_clk or negedge vid_rstn) begin
		if (!vid_rstn) begin
			...
			vid_active_video <= pix_cnt >= H_FRONT + H_PULSE + H_BACK && line_cnt >= V_PULSE / 2 + V_BACK && line_cnt < V_PULSE / 2 + V_BACK + V_DISP;
			vid_hsync <= (pix_cnt >= H_FRONT && pix_cnt < H_FRONT + H_PULSE) ? H_POL : ~H_POL;
			vid_vsync <= (line_cnt >= V_PULSE / 2 && line_cnt < V_PULSE / 2 + V_BACK + V_DISP + V_FRONT) ? ~V_POL : V_POL;
		end
	end
